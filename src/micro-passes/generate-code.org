  ...
  (unless (or (kernel-func-p f) (builtin-func-p f))
    (with-slots (mcl-type id params code-block) f
      (emit "~a ~a(~a) ~a" (gen mcl-type gen)
	    ...
	    (gen-params params gen)
	    (if (and (member (name id) (entry-funcs gen) :test #'string=))
		(gen-entry-code-block-cuda code-block params gen)
		(gen code-block gen))))))
#+end_src
...
(defun gen-entry-code-block-cuda (cb params gen)
  (with-slots (stats) cb
    (with-slots (kernels-file codegen-info) gen
      (let ((params-to-transform (remove-if-not #'array-type-param-p params)))
	(break)
	(emit "{"
	      "    try {"
	      ""
...
#+begin_src lisp
(defun gen-allocate/transfer-cuda (params allocate/transfer gen)
  (assert (eql (length params) 1))
  (let ((param (first params)))
    (emit "opencl::~a(~a, ~a, ~a)"
	  allocate/transfer
