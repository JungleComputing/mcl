...
#+begin_src lisp
(defun generate-code (mcl-file-name output-target hardware-target entry-funcs output-dir)
  (run-pass 'generate-code (list (cons 'semantic-analysis mcl-file-name)
				 (cons 'translate hardware-target)
				 (cons 'generate-code
				       (list output-target hardware-target entry-funcs
                                             output-dir)))))

(defun gen ()
  (generate-code "input/mcpl/vectoradd/vectoradd.mcl" "cpp-cuda" "fermi" (list "vectoradd")
