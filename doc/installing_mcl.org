...
This script has to bootstrap something in ASDF that loads Common Lisp code.
Because of this, this command may fail.  In that case, we can try another time
as ASDF then may have been bootstrapped.  Possibly, the script has to run an
additional time.
